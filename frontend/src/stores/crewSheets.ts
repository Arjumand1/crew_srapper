  ...
  actions: {
    async fetchCrewSheets() {
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to fetch crew sheets';
    ...
    async fetchCrewSheet(id: number) {
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to fetch crew sheet';
    ...
    async uploadCrewSheet(formData: FormData) {
      ...
      try {
        ...
        this.error = error.response?.data?.detail || 'Failed to upload crew sheet';
    ...
    async processCrewSheet(id: number) {
      this.loading = true;
      this.error = null;
      
      try {
        const authStore = useAuthStore();
        await axios.post(`${API_URL}/crew-sheets/${id}/process/`, {}, {
          headers: {
            Authorization: `Bearer ${authStore.accessToken}`
          }
        });
        ...
        this.error = error.response?.data?.detail || 'Failed to process crew sheet';
